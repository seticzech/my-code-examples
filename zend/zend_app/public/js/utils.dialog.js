...
$(document).ready ( function ()
{
	...
	$("#city").autocomplete ( "/crm/address/hint/name/city", {
		width: 320,
		highlight: false,
		scroll: true,
		scrollHeight: 300,
		formatItem: function ( row, i, n, value ) {
			...
			var cityPart = (value.split ("::")[1]) ? " - " + value.split ("::")[1] : "";
		
			return value.split ("::")[0] + cityPart + " " + value.split ("::")[2]
		},
		formatResult: function ( row, value ) {
			return value.split ("::")[0]
		},
		extraParams: {
	       id_country: $("#id_country:selected").val ()
		}
	} ); 
	...
	$("#postcode").autocomplete ( psc, {
		...
		formatItem: function (row, i, n, value) {
			...
			var cityPart = (row.city_part) ? " - " + row.city_part : "";
			
			return row.postcode + " " + row.city + cityPart;
		},
		formatResult: function (row, value) {
			return row.postcode;
		},
		extraParams: {
	       id_country: function () { return $("#id_country:selected").val (); }
		}
	} );
	
	$('#postcode').result ( function ( event, row, formatted ) {
		$("#city").val ( row.city );
		$("#city_part").val ( row.city_part );
	} ).blur (function () {
	    //$(this).search ();
	} );
